  ...
  ngOnInit(): void {
    this.crewFormGroup = this.formBuilder.group({
      crew: this.formBuilder.array(
        this.data.crew.map((member) =>
          this.formBuilder.control({ value: member.username, disabled: false })
        )
      ),
    });
    ...
    if (this.crewFormGroup.valid) {
      const editedCrewUuids = this.getCrewControls().map((_control) => {
        const index = this.userKeys.findIndex(
          (_userKey) => _userKey.username === _control.value
        );
      ...
      toChange.forEach((_change) => {
        this.personalShiftsService
          .getPersonalShifts(this.yearMonth, _change.uuid)
          .pipe(
            filter((_personalShifts) => _personalShifts !== null),
            map(
              (_personalShifts) => _personalShifts as PersonalShifts | undefined
            ),
            first()
          )
          .subscribe((_personalShifts) => {
            if (_personalShifts === undefined) {
              // create
              this.personalShiftsService.createPersonalShifts(this.yearMonth, {
                uuid: _change.uuid,
              ...
              if (_change.action === 'add') {
                ...
                // action === remove
                const index = _personalShifts.shiftUuids.findIndex(
                  (_uuid) => _uuid === this.data.shift.uuid
                );
                _personalShifts.shiftUuids.splice(index, 1);
              }
              this.personalShiftsService.updatePersonalShifts(
                this.yearMonth,
                _personalShifts
              );
            }
      ...
      //check crew full or not
      if (this.data.shift.crewUuids.length < this.data.shift.attendance) {
        this.data.shift.full = false;
      } else {
        this.data.shift.full = true;
      }
